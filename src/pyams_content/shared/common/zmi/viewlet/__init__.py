...
class SharedContentsMenu(TopMenuViewletManager):
    ...
    def update(self):
        ...
        for name, tool, label in sorted((
                (
                        name,
                        tool,
                        get_object_label(tool, request=request)
                )
                for name, tool in filter(lambda x: (not ISharedSite.providedBy(x[1])) and
                                                   x[1].shared_content_menu,
                                         get_utilities_for(ISharedTool))
        ), key=lambda x: locale.strxfrm(x[2])):
            if not name:
                continue
            self.add_menu(context, request, parent, tool, label)
